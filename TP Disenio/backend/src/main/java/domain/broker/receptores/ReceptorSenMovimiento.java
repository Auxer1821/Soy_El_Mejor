...
@Getter
@Setter
public class ReceptorSenMovimiento extends ReceptorPersistente {
    private SensorMovimiento sensorMovimiento;

    public ReceptorSenMovimiento(SensorMovimiento sensor) {
        setId(UUID.randomUUID().toString());
        this.sensorMovimiento = sensor;

        try {
            setClient(new MqttClient("tcp://broker.hivemq.com:1883",
                    this.getId(), new MemoryPersistence()));
            ...
            if (getClient().isConnected()) {
                getClient().subscribe(
                        "heladera/" + this.sensorMovimiento.getHeladera().getId() + "/sensores/alertas/fraude");
            } else {
